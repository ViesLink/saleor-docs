...
```
- If you run command 'docker compose build' and an error message is reported about version, comment out or delete the line where the version claim is located in the .yml file
...
- While You are using the WSL 2 backend, so resource limits are managed by Windows.(You can skip the current step)
